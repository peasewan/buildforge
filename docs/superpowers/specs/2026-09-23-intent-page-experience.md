...
- Rollout is per pathname, not a global switch by page kind. Remaining pages must retain prior markup/layout until their own batch.
- Leveling sequence is editorial, not client fact. Each rank increment must be legal; preserve source point order where available. Never infer precise source ordering from a final allocation without labeling derived ordering.
- Data-incomplete modules show verified limits and a useful supported task. The core requirement is useful distinct behavior, not a fictional guide.
- Changed pages receive truthful lastmod only upon publication; sitemap URL set stays 150. GSC submission is not a guarantee of indexing.
