...
import { renderBetaAvailabilityPrerender, renderBetaLevelingSnapshotPrerender, renderBetaSpecPathPrerender, renderBetaStatusPrerender } from '../src/lib/prerender'
...
for (const [contentFile, outputFile] of buildTargets) {
  ...
  const buildPrerendered = `<main class="build-page build-prerender">
    <article>
      ...
      ${contentFile.includes('retribution-leveling-build') ? '' : renderBetaSpecPathPrerender(build.spec.toLowerCase() as Branch)}
